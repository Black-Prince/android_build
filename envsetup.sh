...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^black_") ; then
       BLACK_BUILD=$(echo -n $1 | sed -e 's/^black_//g')
    else
       BLACK_BUILD=
    fi
    export BLACK_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/black/vendorsetup.sh 2> /dev/null`
        do
echo "including $f"
            . $f
        done
...
echo "z$target" | grep -q "-"
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BlackSRom model name
            lunch black_$target-userdebug
        fi
fi
return $?
}
